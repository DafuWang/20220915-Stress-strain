function [A_average,beta_average,b_average,ultimate_stress_average,ultimate_strain_average,R2_average,save_adress_name,figure_name]=fitting_stress_average_strain_with_b_beta(fileadress,filename,NUM_joint_average,I)
...
initial_a_joint=[5,1];%initial_a(1)为beta值，initial_a(2)为b
...
[x,y_fitting,beta_average(i),b_average(i),R2_average(i),I,I11(i),fig] = strain_stress_fitting_b_beta(filename,r_stress,strian,I,data_legnend_joint,fiting_legend_joint,initial_a_joint,ultimate_strain_average(i),i);
A_average{i,1}=[x,r_stress,y_fitting];initial_a_average=[beta_average(i),b_average(i)];
end
figure_name{1,1}='averrage stress strain fitting with b-beta';
